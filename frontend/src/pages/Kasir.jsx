      ...
      <div className="w-2/3 flex flex-col">
        ...
        <div className="bg-white p-6 border-b border-gray-200 flex justify-between items-center shadow-sm z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-doabunda-dark tracking-wide">MESIN KASIR</h1>
              <p className="text-sm text-gray-500 font-medium">RM. Doa Bunda</p>
            </div>
          </div>
          ...
          <div className="relative w-72">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-gray-100 border-transparent rounded-xl focus:bg-white focus:border-doabunda-primary focus:ring-2 focus:ring-doabunda-primary/20 outline-none transition-all text-sm font-medium"
          ...
          <div className="grid grid-cols-3 gap-6">
            {menus.filter(m => m.name.toLowerCase().includes(searchTerm.toLowerCase())).map((menu) => (
              <button 
                ...
                className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-doabunda-primary/30 transition-all text-left group flex flex-col justify-between h-32 active:scale-95"
              >
                <div>
                  <h3 className="font-bold text-gray-800 line-clamp-2 leading-tight group-hover:text-doabunda-primary transition-colors">{menu.name}</h3>
                  <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase mt-2 inline-block">{menu.category}</span>
                </div>
            ...
            cart.map((item) => (
              <div key={item.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl border border-gray-100">
                <div className="flex-1 pr-4">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Rp {item.price.toLocaleString('id-ID')} x {item.qty}</p>
                </div>
                <p className="font-bold text-doabunda-dark">Rp {(item.price * item.qty).toLocaleString('id-ID')}</p>
              </div>
            ))
          )}
        </div>
