...
const SidebarAdmin = () => {
  ...
  return (
    <>
      {/* Header Menu HP */}
      <div className="md:hidden fixed top-0 left-0 w-full bg-doabunda-dark text-white h-16 flex justify-between items-center px-4 z-50 shadow-md">
      ...
      {/* Latar Gelap HP */}
      {isOpen && <div className="fixed inset-0 bg-black/60 z-40 md:hidden backdrop-blur-sm" onClick={() => setIsOpen(false)} />}

      {/* Sidebar Full Height Tanpa Ruang Putih */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-doabunda-dark text-white flex flex-col h-[100dvh] shrink-0 shadow-2xl transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="hidden md:block p-8 text-center border-b border-white/10 shrink-0">
        ...
        <div className="h-16 md:hidden border-b border-white/10 flex items-center justify-between px-4 bg-doabunda-dark shrink-0">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)}
                className={`flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all ${isActive ? 'bg-doabunda-primary text-white font-bold shadow-lg' : 'text-white/70 hover:bg-white/10 hover:text-white font-medium'}`}
              >
