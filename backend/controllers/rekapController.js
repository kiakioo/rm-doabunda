...
const generateDailyRecap = async (req, res) => {
    try {
        ...
        const [expenseResult] = await db.query(
            'SELECT SUM(amount) as total FROM expenses WHERE expense_date = ?', [today]
        );
        ...
        await db.query(
            `INSERT INTO daily_recaps (admin_id, recap_date, total_cash, total_qris, total_grab, total_revenue, total_expense, net_profit) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
const getSummary = async (req, res) => {
    try {
        ...
        res.json({ totalRevenue, totalTransactions });
    } catch (error) {
...
module.exports = { generateDailyRecap, getSummary, getHistory };